...
class VLMVLM(SimilarityVLM):
    """
    Similarity-based VLM that uses VLM for frame and text encoders.  This uses our own modification of the FAIR
    repository MMPT (original repo link is here: https://github.com/facebookresearch/fairseq/tree/main/examples/MMPT).
    """

    def __init__(self, path: str = "model/videoclip/MMPT_updated/projects/mtm/vlm/how2.yaml",
    ...
    def load_model(self, path="video_clip/MMPT_updated/projects/mtm/vlm/how2.yaml"):
        ...
        """
        print("PATH IS:",
              path)  # /home/zaned/code/vlm_benchmark/video_clip/MMPT_updated/projects/retri/videoclip/how2.yaml
        ckpt_save_dir = path[:path.rfind("/")]  # Files stored in retri/videoclip repo
        ckpt_save_dir = ckpt_save_dir.replace("projects", "runs")
        print("CKPT SAVE DIR:",
              ckpt_save_dir)  # /home/zaned/code/vlm_benchmark/video_clip/MMPT_updated/projects/retri/videoclip
    ...
    def tokenize(self, text):
        ...
        for t in text:
            caps, cmasks = self.model.aligner._build_text_seq(
                self.model.tokenizer(t, add_special_tokens=False)["input_ids"])
            caps, cmasks = caps[None, :], cmasks[None, :]
    ...
    def text_encoder_from_word_embeddings(self, input_word_embeds: torch.Tensor,
        ...
        text_output_attn_mask = text_output_attn_mask.type(text_outputs.dtype) / text_output_attn_mask.sum(1,
                                                                                                           keepdim=True)
    ...
    def sample_frame_indices(self, video_len: int, video_fps: float, subvideo_start_frame: Optional[int] = None,
                             subvideo_end_frame: Optional[int] = None, random_augment: bool = False) -> np.ndarray:
        ...
        if self.sample_strat == "center":
            ...
            focus_start_native = max(
                subvideo_start_frame + total_framecount_native // 2 - focused_framecount_native // 2,
                subvideo_start_frame)
            ...
            focus_frame_indices_desired = np.minimum(
                np.round(np.linspace(focus_start_native, focus_end_native, focused_framecount_desired, endpoint=False)),
                subvideo_end_frame - 1)
        ...
        if self.sample_strat == "spread":
            block_frame_starts_native = np.round(
                np.linspace(subvideo_start_frame, subvideo_end_frame, focused_seconds, endpoint=False))
            if random_augment:
                block_frame_starts_native += np.random.choice(
                    int((subvideo_end_frame - subvideo_start_frame) / (2 * focused_seconds)),
                    size=focused_seconds)  # Randomly start blocks up to half way towards the start of the subsequent block
            focus_frame_indices = []
