...
class FewShotTaskDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(self, query_dataset: DatasetHandler, support_dataset: DatasetHandler, n_episodes: int, n_way: int,
                 n_support: int, n_query: Optional[int], val_tuning_dataset: Optional[DatasetHandler]) -> None:
        super().__init__()
        ...
        if self.same_dataset:
            ...
            self.category_names = [name for name, vids in query_dataset.data_dict.items() if
                                   len(vids) >= min_category_vids]
        else:
            min_query_vids = n_query or 1
            min_support_vids = n_support
            self.category_names = [
                name for name in query_dataset.data_dict
                if name in support_dataset.data_dict and len(query_dataset.data_dict[name]) >= min_query_vids and len(
                    support_dataset.data_dict[name]) >= min_support_vids
            ]

        if len(self.category_names) < n_way:
            raise ValueError(
                f"Only {len(self.category_names)} valid categories for few-shot tasks with the given parameters. Needs at least {n_way} categories for {n_way}-way few-shot tasks.")
        ...
        for i in range(self.n_episodes):
            ...
            for cat_label, cat_ind in enumerate(sampled_categories):
                if self.same_dataset:
                    ...
                    sampled_queries = self.rng.choice(self.query_videos[cat_ind],
                                                      size=(self.n_query or len(self.query_videos[cat_ind])),
                                                      replace=False)
                    query_videos += sampled_queries.tolist()
                    query_labels += [cat_label] * len(sampled_queries)

                    if support_videos is not None:
                        sampled_supports = self.rng.choice(self.support_videos[cat_ind], size=self.n_support,
                                                           replace=False)
                        support_videos.append(sampled_supports)
            ...
            if self.val_tuning_videos is None:
                ...
                val_tuning_videos = [self.val_tuning_videos[cat_ind] for cat_label, cat_ind in
                                     enumerate(sampled_categories)]
                val_tuning_labels = [[cat_label] * len(self.val_tuning_videos[cat_ind]) for cat_label, cat_ind in
                                     enumerate(sampled_categories)]
