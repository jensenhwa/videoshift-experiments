...
class NameTuningFewShotClassifier(FewShotClassifier):
    def __init__(self, vlm: SimilarityVLM, prompt_ensemble_id: Optional[str] = None, name_regularization: float = 1e-2,
                 lr: float = 1e-3, epochs: int = 20, warmup_lr: float = 1e-5, warmup_epochs: int = 1,
                 batch_size: int = 1, optimizer: str = "sgd", random_augment: bool = True,
    ...
    def predict(self, category_names: np.ndarray, support_video_paths: Optional[np.ndarray],
                query_video_paths: np.ndarray,
                val_tuning_video_paths: Optional[np.array] = None,
                val_tuning_video_labels: Optional[np.array] = None) -> np.ndarray:
        ...
        if n_support == 0:
            ...
            self.query_class_probabilities = np.exp(query_class_logits) / np.sum(np.exp(query_class_logits), axis=1,
                                                                                 keepdims=True)
        ...
        for epoch_idx in range(self.epochs):
            ...
            for batch_idx, (vid_paths, vid_labels) in enumerate(train_dataloader):
                ...
                with torch.no_grad():
                    total_reg_loss += 0.5 * self.name_regularization * module.name_perturbation.pow(2).sum() * len(
                        vid_paths)
            ...
            if val_tuning_dataloader is not None:
                ...
                print(
                    f"Epoch {epoch_idx:5}: Support Acc = {total_correct / total_count:5.3f}, Val-Tune Acc = {total_val_correct / total_val_count:5.3f}, Loss = {total_loss / total_count:5.3E}, Reg Loss = {total_reg_loss / total_count:5.3E}, Combined Loss = {(total_loss + total_reg_loss) / total_count:5.3E}")
            else:
                print(
                    f"Epoch {epoch_idx:5}: Support Acc = {total_correct / total_count:5.3f}, Loss = {total_loss / total_count:5.3E}, Reg Loss = {total_reg_loss / total_count:5.3E}, Combined Loss = {(total_loss + total_reg_loss) / total_count:5.3E}")
        ...
        query_dataloader = torch.utils.data.DataLoader(query_video_paths, batch_size=QUERY_BATCH_SIZE, num_workers=0,
                                                       shuffle=False)
        with torch.no_grad():
            query_logits = []
...
class NameTuningModule(nn.Module):
    def __init__(self, vlm: SimilarityVLM, category_names: np.ndarray, prompt_templates: List[str]):
        ...
        with torch.no_grad():
            category_name_input_embeds, category_name_attn_masks = vlm.get_input_word_embeddings(
        ...
        prompt_name_input_embeds = prompt_name_input_embeds.reshape(len(prompt_templates), len(category_names), seq_len,
                                                                    word_embed_dim)
        ...
        with torch.no_grad():
            ...
            self.pre_name_template_lengths = [
                vlm.get_input_word_embeddings([pre_name_template])[0].shape[
                    1] - self.vlm.text_start_special_token_count() - self.vlm.text_end_special_token_count()
