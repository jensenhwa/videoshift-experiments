...
for test_params in pbar:
    ...
    if args.method in ["gp", "forest"]:
        ...
        skopt_results = skopt_func(val_neg_accuracy, hyperparam_space, n_calls=args.n_search_runs,
                                   callback=skopt_callback, x0=x0, y0=y0)
        ...
        if runs_per_category_choice == 0:
            raise ValueError(
                f"Too many categorical hyperparameters to iterate over all choices without exceeding {args.n_search_runs} runs.")
        ...
        if len(other_hyperparams) == 0:
            ...
            if samples_per_cont_hyper == 0:
                raise ValueError(
                    f"Too many hyperparameters to iterate over all categories and still choose multiple values per continuous space, without exceeding {args.n_search_runs} runs.")
            ...
            for hyper in hyperparam_space:
                if type(hyper) is skopt.space.space.Categorical:
                    ...
                    if hyper.prior == "log-uniform":
                        hyper_samples = np.logspace(np.log10(hyper.low), np.log10(hyper.high),
                                                    num=samples_per_cont_hyper, endpoint=True)
                    else:
    ...
    if args.method != "grid":
        ...
        dummy_results = skopt.gp_minimize(val_neg_accuracy, hyperparam_space, n_calls=0, n_initial_points=0, x0=x0,
                                          y0=y0)
        ...
        if isinstance(ax, np.ndarray):
            ...
            fig = ax.get_figure()
        fig.savefig(
            os.path.join(RESULTS_FOLDER, ".".join(
                [query_dataset_val.id()] + [f"{key}_{val}" for key, val in test_kwargs.items()] + ["jpg"])),
