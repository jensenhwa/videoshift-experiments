...
class FewShotTestHandler:
    ...
    def run_few_shot_test(self, classifier: FewShotClassifier, query_dataset: DatasetHandler,
                          support_dataset: DatasetHandler,
        ...
        if test_already_stored(self.results, classifier, query_dataset, support_dataset, n_way, n_support, n_query,
                               n_episodes, val_tuning_dataset):
            # Find dataframe row with accuracy from this run
            filter_indices = np.ones(len(self.results))
            for key, val in dataframe_format(classifier, query_dataset, support_dataset, n_way, n_support, n_query,
                                             n_episodes, val_tuning_dataset).items():
                if pd.isna(val):
        ...
        self.results = append_test_result(self.results, classifier, query_dataset, support_dataset, n_way, n_support,
                                          n_query, n_episodes, val_tuning_dataset, accuracy, accuracy_std)
...
def test_already_stored(results: pd.DataFrame,
                        ...
                        n_way: int, n_support: int, n_query: int, n_episodes: int,
                        val_tuning_dataset: Optional[DatasetHandler]) -> bool:
    valid_indices = np.ones(len(results)).astype(bool)
    for key, val in dataframe_format(classifier, query_dataset, support_dataset, n_way, n_support, n_query, n_episodes,
                                     val_tuning_dataset).items():
...
def append_test_result(results: pd.DataFrame,
    ...
    formatted_row = dataframe_format(classifier, query_dataset, support_dataset, n_way, n_support, n_query, n_episodes,
                                     val_tuning_dataset, accuracy, accuracy_std)
    ...
    if len(new_columns):
        ...
        sorted_columns = ["vlm_class"] + sorted_vlm_param_columns + [
            "classifier_class"] + sorted_classifier_param_columns + \
                         ["query_dataset", "support_dataset", "n_way", "n_support", "n_query", "n_episodes",
                          "val_tuning_dataset", "accuracy", "accuracy_std"]
...
def optimize_hyperparameters(results: pd.DataFrame,
    ...
    if "classifier.text_weight" in hyperparam_cols and "n_support" not in group_by_cols and (
            output["n_support"] == 0).sum() == 0:
        line_identifiers = output.drop_duplicates(group_by_cols + hyperparam_cols).drop(
            columns=target_cols + average_over_cols)
